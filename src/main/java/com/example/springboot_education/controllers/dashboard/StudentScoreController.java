    ...
    public ResponseEntity<?> getRecentScores(HttpServletRequest request) {
        try {
            ...
            if (authHeader == null || !authHeader.startsWith("Bearer ")) {
                return createErrorResponse("Authorization header missing or invalid format", HttpStatus.UNAUTHORIZED);
            }
            ...
            if (studentId == null) {
                return createErrorResponse("Cannot extract user ID from token", HttpStatus.BAD_REQUEST);
            }
...
//            }
//
//            List<ClassResultDTO> classResults = studentScoreService.getAllClassResults(studentId);
...
//    }
    ...
    public ResponseEntity<?> testToken(HttpServletRequest request) {
        try {
            ...
            return createErrorResponse("Error testing token: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
